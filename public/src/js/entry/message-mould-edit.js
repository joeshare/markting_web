        ...
        return (
            <header className="page-body-header">
                <div className="text-box">
                    <span className="title">模板管理-编辑</span>
                </div>
...
class Manage extends React.Component {
    ...
    showHideHint(param){
        ...
        let textarea = $('#template-textarea').val().trim(),textareaJudgment = false;
    ...
    fetch(){
        let that = this;
        let nameHint = '',type = '固定模板',mouldType = '',textarea = '',huitBoxShowClass = '',textareaLength = 0,variableClass = '',fetchVariable = [],stateVariable = [],variableList=[];
        let thisData="";
        ...
        util.api({
            ...
            success: function (res) {
                if(res.code == 0){
                    ...
                    $('#materialName').val(thisData.name);
                    $('#template-textarea').val(textarea);
                    fetchVariable = thisData.material_list;
                    if(fetchVariable.length > 0){
                        variableClass = ' show';
                        for(let i=0; i<fetchVariable.length; i++){
                            variableList[i] = {
                                nom:fetchVariable[i].sms_variable_value,
                                name:fetchVariable[i].material_name+'—'+fetchVariable[i].material_property_name
                            };
                            stateVariable[i] = {
                                nom:fetchVariable[i].sms_variable_value,
                                tabId:fetchVariable[i].material_id,
                                tabType:fetchVariable[i].material_type,
                                tabName:fetchVariable[i].material_name,
                                listId:fetchVariable[i].material_property_id,
                                listCode:fetchVariable[i].material_property_code,
                                listName:fetchVariable[i].material_property_name
                            }
                        }
                    ...
                    that.setState({
                        id:thisData.id,
                        mouldNum:thisData.channel_type,
                        mouldType:mouldType,
                        name:thisData.name,
                        nameHint:nameHint,
                        type:type,
            ...
            buttons: [
                {
                    ...
                    handler: function (self) {
                        ...
                        let variable = [],variableList = [],variableNom;
                        if(!optsButtons0Cls){
    ...
    saveButton(){
        ...
        if(this.state.saveButtonClass == ""){
            channel = this.state.mouldNum;
            name = this.state.name;
            thisType = this.state.type;
            textarea = this.state.textarea;

            if(thisType == '固定模板'){type = 0;materialList = [];}
            if(thisType == '变量模板'){
                type = 1;
    ...
    /*
     variableClass:' show',
     variable:[
     {nom:1,tabId:1,tabType:'coupon',tabName:'优惠券',listId:'1',listCode:'amount',listName:"价值"}
     ],
     variableList:[
     {nom:1,name:'优惠券-优惠码'}
     ],
                                ...
                                <div className="market">
                                    ...
                                    <div className="line trigger">
                                        <div className="line-con nan-title">
                                            <div className={"button-main-1 save"+this.state.saveButtonClass} onClick={this.saveButton.bind(this)}>提交</div>
                                            <a className="button-assist-1 reset" href={BASE_PATH+"message-mould.html"}>取消</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
