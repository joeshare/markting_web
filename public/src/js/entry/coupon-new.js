        ...
        return (
            <header className="page-body-header">
                <div className="text-box">
                    <span className="title">新建优惠券</span>
                </div>
...
class PlayOne extends React.Component{
    ...
    nextButton(){
        let param;
        let nowParam = {oneParam:'',twoParam:'',threeParam:'',nowStep:'',nowTime:''};
        ...
        if(this.state.nextButtonClass==''){
            ...
            nowParam.oneParam = param;
            nowParam.nowStep = 'two';
            nowParam.nowTime = nowHistoryState.nowTime;
            history.pushState(nowParam,'','');
            this.props.stepRecord('two');
        }
    }
    constructor(props){
        /*common:普通码;generate:平台生成码;own:自有码*/
        super(props);
        this.state = {
            couponName:'',
            couponType:'common',
        ...
        return (
            <div className="play-one">
                ...
                <div className="line type">
                    <div className="line-title"><span className="redstar">&#42;</span>卷吗类型</div>
                    <div className="line-cont">
                        <div className="radio-area">
                            ...
                            <div className="radio-box">
                                <input className="type1" name="couponType" type="radio" id="couponType2" checked={this.state.couponRadio[1]} onClick={this.isChecked.bind(this,'generate')}/>
...
class PlayTwoCommon extends React.Component{
    ...
    nameTest(){
    let param;
    let nametf = false,hintName;
    let name = $('#common-name').val().trim() || false;
    let nameReg = new RegExp(/[`~!@#\$%\^\&\*\(\)\+<>\?:"\{\},\.\\\/;'\[\]]/im);
    if(name){
        if(nameReg.test(name)){
            nametf = false;
            hintName = {show:'block',order:3};
        }else{
            nametf = true;
            hintName = {show:'none',order:0};
        }
    }else{
        nametf = false;
        hintName = {show:'block',order:0};
    }
    param = {hint:hintName,tf:nametf};
    return(param);
}
    numTest(){
        let param;
        let numtf = false,hintNum;
        let num = $('#common-num').val().trim() || false;
        let numReg = new RegExp(/\d+/);
        if(num){
            if(numReg.test(num)){
                numtf = true;
                hintNum = {show:'none',order:0};
            }else{
                numtf = false;
                hintNum = {show:'block',order:2};
            }
        }else{
            numtf = false;
            hintNum = {show:'block',order:0};
        }
        param = {hint:hintNum,tf:numtf};
        return(param);
    }
    buttonClass(testType){
        let testName, testNum;
        let nametf = false,numtf = false,hintName,hintNum;

        switch (testType){
            case 'init':
                hintName = {show:'none',order:0};
                hintNum = {show:'none',order:0};
                this.setState({hintName:hintName,hintNum:hintNum});
                break;
            case 'name':
                testName = this.nameTest();
                testNum = this.numTest();
                nametf = testName.tf;
                numtf = testNum.tf;
                hintName = testName.hint;
                this.setState({hintName:hintName});
                break;
            case 'num':
                testName = this.nameTest();
                testNum = this.numTest();
    ...
    constructor(props){
        ...
        this.state = {
            ...
            commonNum:'',
            nextButtonClass:' disable',
            hint:['不能为空','请按规则填写','只能是数字','数字、英文、中文、减号、下划线'],
            interim:false,
            hintName:{show:'none',order:0},
            hintNum:{show:'none',order:0}
        };
                ...
                <div className="common">
                    <div className="line name">
                        ...
                        <div className="line-cont">
                            ...
                            <div className="huit-icon iconfont">&#xe66f;</div>
                            <div className="hint" style={{'display':this.state.hintName.show}}>券码名称{this.state.hint[this.state.hintName.order]}</div>
                        </div>
                    </div>
                    <div className="line amount">
                        ...
                        <div className="line-cont">
                            ...
                            <div className="hint" style={{'display':this.state.hintNum.show}}>券码数量{this.state.hint[this.state.hintNum.order]}</div>
                        </div>
                    </div>
                </div>
                <div className="line button">
...
class PlayTwoGenerate extends React.Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            couponNum:'',
            nextButtonClass:' disable',
            hint:['券码数量不能为空','只能是数字','数值不能超过可生成的卷码量'],
            hintValue:{show:'none',num:0}
        };
    }
    fetch(){
        ...
        if(twoParam){
            ...
            param = {outcomeNum:5,couponNum:num};
            this.setState({
                ruleOne:ruleOne,
                ruleOneRadio:radio,
                ruleTwo:ruleTwo,
                outcomeNum:param.outcomeNum,
                couponNum:num
            });
            $('#coupon-num').val(num);
                                ...
                                return (
                                    <div className="lists">
                                        ...
                                        <div className="detail">
                                            ...
                                            <div className="delete" onClick={this.fileDelete.bind(m.name)}>删除</div>
                                        </div>
                                    </div>
                                )
                            })}
...
class PlayThree extends React.Component{
    changePrice(){
        let price = $('#coupon-price').val().trim();
    ...
    endTime(){
        let that =this;
        let thisDate;
        let minTime = this.state.minTime,startTime = this.state.startTime || false;
        if(startTime){minTime = startTime}
        $('#end-time').pickadate({
            format: 'yyyy-mm-dd',
            selectMonths: true,
            selectYears: 5,
    ...
    fetch(){
        ...
        util.api({
            ...
            success: function (res) {
                if(res.code == 0){
                    that.setState({minTime:res.date});
                }
            }
        });
        if(threeParam){
            let price = threeParam.price;
            $('#coupon-price').val(price);
            this.setState({
                price:price,
                startTime:threeParam.startTime,
                endDate:threeParam.endDate
            });
            this.buttonClass('init');
        }else{
            this.buttonClass();
...
class Manage extends React.Component{
    ...
    fetch(){
        let that = this;
        let nowHistoryState = history.state || false, paramStep = 'one';
