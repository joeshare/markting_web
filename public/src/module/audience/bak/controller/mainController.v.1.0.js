...
var msg1='受众标签至少有一个，才可发布';
var msg2='发布状态修改，受众标签至少有一个';
var API = {
...
function controller() {
    ...
    function showEditTag(type) {
        if (type) {
            $('#aud-tag-edit').removeClass("rui-disabled");
        } else {
            $('#aud-tag-edit').addClass("rui-disabled");
        }
    ...
    function saveMainInfo(id) {
        ...
        util.api({
            ...
            success: function (res) {
                ...
                current_status==1&&pageReleaseStatusSet();
            },
            error: function (res) {
                errorAlertMsg("保存失败！");
                editBtnDisabled(false);
    ...
    function saveSegmentInfo(name, status) {
        util.api({
            url: CURRENTSEGMENTID ? API.updateHeaderInfo : API.createHeaderInfo,
            type: 'post',
            data: {
                "segment_head_id": CURRENTSEGMENTID,
                "segment_name": name,
            ...
            success: function (res) {
                ...
                if (res && res.code == 0) {
                    ...
                    current_status=rec['release'] = status ? status : 0;
                    rec['name'] = name;
                    if (!CURRENTSEGMENTID) {//为空表示新建，非空表示修改操作
                        CURRENTSEGMENTID = rec.id;
                        showEditTag(true);
                    }
                    changeSegmentTitle(rec);
                    //保存主数据
                    saveMainInfo(CURRENTSEGMENTID);
                }else if(res && res.code == 3003){//活动中的细分无法编辑 错误码是 3003
                    releaseActiveStatusShow();
                    errorAlertMsg(res.msg);
                } else {
    ...
    //页面发布状态设置
    function pageReleaseStatusSet(){
        saveBtnDisabled(true);
        conditionOptStatusSet(false)
    }
    //页面编辑状态设置
    function pageEditStatusSet(){
        saveBtnDisabled(false);
    ...
    function changeSegmentTitle(arg) {
        var release = arg.status || $('#audience-release')[0] && $('#audience-release')[0].checked;
    ...
    this.changeTotal = function () {
        var charts=_this.charts.getCharts();
        var t=0;
        for(var chartId in _this.charts.getCharts()){
            if(chartId!='bar1'){
               var chart= charts[chartId];
                chart.getOption().series[0].data.forEach(function(itm,i){
                    t+=itm.value;
                })
            }
        }
        ...
        events = {
            ...
            "click #rui-modals-save": function (e) {
                ...
                if (!$(e.target).hasClass("rui-disabled")) {
                    ...
                    saveSegmentInfo(name, current_status)
                }
                saveBtnDisabled(true);
            },
                ...
                var win=new Modals.Window({
                    ...
                    buttons: [{
                        text: '保存', cls: 'accept '+isSaveBtnDisabed, handler: function (thiz) {
                            ...
                            var release = ($('#audience-release')[0] && $('#audience-release')[0].checked) ? 1 : 0;
                            var name = $('#audience-name').val();
                            !$.trim(name) && (name = '未命名-'+getDateStr());
                            saveSegmentInfo(name, release);
                            if(release!=1){
                                pageEditStatusSet();
                            }
                            thiz.close();
        ...
        util.api({
            ...
            success: function (res) {
                ...
                if (res && res.code == 0 && res.data && res.data.length) {
                    ...
                    current_status > 1&&doRelease();
                    changeSegmentTitle(rec);
                    CURRENTSEGMENTID = id;
                    //打开添加关联标签功能
                    showEditTag(true);
                    queryMainInfo(CURRENTSEGMENTID,function(){
                         current_status > 1 &&releaseActiveStatusShow();
